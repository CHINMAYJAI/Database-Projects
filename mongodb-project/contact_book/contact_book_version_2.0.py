...
connectionEstablish = MongoClient(mongodb_connection) # TODO: place mongodb connecting string in the brackets
...
def SeeingExistingContact():
    """This function is used to watch the contact details which the user wants to see"""
    person_name = str(input('Enter the person name: '))
    query = {"Name":person_name.title()}
    tracked_person = collection.find_one(query)
    if tracked_person:
        print('Match found ✅')
...
def ContactBook():
    while True:
        ...
        elif "se" in user_choice.lower():
            SeeingExistingContact()
            ExitingFromFunction()
            break
        else :
            print("Invalid Input!")
